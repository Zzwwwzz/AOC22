...
def readFile():
    grid_text, path = open("i/22.txt").read().split('\n\n')
    grid = {}
...
def main():
    ...
    print(f'Part 1: {int(1000 * (1 + final_pos.imag) + 4 * (1 + final_pos.real) + DIR_VALS[dp])}')
    final_pos, dp = run(teleport_2, grid, SIZE, 1, path)
    print(f'Part 2: {int(1000 * (1 + final_pos.imag) + 4 * (1 + final_pos.real) + DIR_VALS[dp])}')
