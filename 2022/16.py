...
def p1(distances, rooms, cur="AA", time=30, seen=set(), targets=None):
    seen = seen | {cur}
    ...
    for target in targets:
        ...
        if time_left > 0:
            ...
            flow += p1(distances, rooms, target, time_left, seen, targets)
            if flow > best_flow:
                best_flow = flow
    return best_flow


def p2(distances, rooms, key_rooms):
    endpoints = {}
...
def main():
    ...
    print("Part 1:", p1(distances, rooms, targets=key_rooms))
    print("Part 2:", p2(distances, rooms, key_rooms))
